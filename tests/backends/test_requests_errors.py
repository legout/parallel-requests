...
@pytest.mark.asyncio
async def test_requests_backend_exception_wrapping():
    ...
    async with RequestsBackend() as backend:
        ...
        assert exc_info.value.backend_name == "requests"
...
@pytest.mark.asyncio
async def test_requests_http2_warning_with_enabled():
    """Test that HTTP/2 warning is issued when enabled for requests."""
    async with RequestsBackend(http2_enabled=True) as backend:
...
@pytest.mark.asyncio
async def test_requests_http2_no_warning_with_disabled():
    """Test that no HTTP/2 warning is issued when disabled for requests."""
    async with RequestsBackend(http2_enabled=False) as backend:
        config = RequestConfig(url="https://httpbin.org/get", method="GET")
...
@pytest.mark.asyncio
async def test_requests_http2_warning_only_once():
...
@pytest.mark.asyncio
async def test_requests_connection_error():
...
@pytest.mark.asyncio
async def test_requests_timeout_error():
...
@pytest.mark.asyncio
async def test_requests_json_decode_error():
    """Test that JSON decode errors are handled gracefully."""
    async with RequestsBackend() as backend:
        # Request non-JSON content but expect JSON
        config = RequestConfig(url="https://httpbin.org/html", method="GET")
        response = await backend.request(config)

        # Response should still be successful, but json_data should be None
        # since content is not valid JSON
        assert response.status_code == 200
